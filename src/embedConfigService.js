 ...
 */
async function getEmbedInfo(username, roles) {
    console.log('getting embed info')
    ...
    try {
        ...
        const result = await getEmbedTokenForSingleReportSingleWorkspace(config.reportId, config.datasetId, username, roles);
 ...
 * @return EmbedToken
 */
async function getEmbedTokenForSingleReportSingleWorkspace(reportId, datasetId, username, roles) {
    ...
    try{
        let datasetId = await client.getSecret('datasetId')
        let reportId = await client.getSecret('reportId')
